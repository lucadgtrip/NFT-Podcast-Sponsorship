...
class ClaimingForm extends Component{
  ...
  async fetchInitialInfo(firstFetch) {
      ...
      try {
          ...
          // let adjustedPrice = parseInt(this.props.state.web3.utils.fromWei(await instance.methods.price().call())).toFixed(4);
          
          let adjustedPrice = this.props.state.web3.utils.fromWei(await instance.methods.price().call()); // ai
  ...
  onMint = async (event) => {
    ...
    try{
      ...
      this.fetchNFTList();
      this.fetchInitialInfo(false); //don't move it under the setState, otherwise the successMessage would be overwritten
      this.setState({successMessage:"🎁 THANK YOU! Minting successfull. Check your donation NFT below ⬇️", errorMessage: ""});
      this.happyShalala();
    }
    catch(err){
      ...
      try{
        ...
        for (let index = 0; index < lastUserIndex; index++){
          ...
          let element = {
            ...
            "image":uri.image,
            "extra":<div className="text-center"><a target="_blank" href={this.state.chain.marketCard.replace("[PH_ADDR]",this.state.chain.addr).replace("[PH_ID]",uri.name.replace("Ticket #",""))}>See it on OpenSea</a></div>,
          };
          all.push(element);
          //console.log(uri);
  ...
  render(){
    return (
      ...
      <h2 className="text-center">You are minting Donation NFT  #{1 + this.state.totalSupply}</h2>
...
<h4>${(this.state.adjustedPrice * this.state.ethUsdPrice).toFixed(2) } at the real time rate of ${this.state.ethUsdPrice} per ETH</h4>


                <div className={`${styles.marginBottom}`} >This is a low as it will ever be: the next donation will cost more</div>
                </Form.Field>
                ...
                <Form.Field>
                  ...
                  <Message warning icon >
                    <Message.Content>
                      <Message.Header>⏳ Pending wallet confirmation...</Message.Header>
              ...
              <br />
              <h3 className="text-center">Your previous donation(s):</h3>
              {
                this.state.all.length == 0
                ?
                <div className="text-center">No donations found. Click on "Mint to donate!" button above to do one now! ❤️</div>
                :
                
                <div style={{padding:"15px"}}  >
                    <Card.Group itemsPerRow={1} stackable={true} doubling={true} centered items={this.state.all}  style={{ width: '100%', height: '80%'}} />
                  </div>
